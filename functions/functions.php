 ...
 */
 ...
 if ( !function_exists( 'cpbw_apply_product_badges' ) ) {
     ...
     * @return html
	 */
	function cpbw_apply_product_badges( $badge, $product ) {
...
if( !function_exists( 'cpbw_add_badge_to_shop_product_image' ) ){
    // Add text overlay on WooCommerce product image using woocommerce_product_get_image filter
    function cpbw_add_badge_to_shop_product_image($image, $product) {
        ...
        $badge = cpbw_apply_product_badges( $image, $product );
    ...
    add_filter('woocommerce_product_get_image', 'cpbw_add_badge_to_shop_product_image', PHP_INT_MAX, 2);
}


if( !function_exists( 'cpbw_add_badge_to_single_product_image' ) ){
    // Add text overlay on WooCommerce product image using woocommerce_single_product_image_thumbnail_html filter
    function cpbw_add_badge_to_single_product_image( $image, $attachment_id ) {
...
// add_filter('woocommerce_single_product_image_thumbnail_html', 'cpbw_add_badge_to_product_image_single', 10, 2);
 ...
 */
if( ! function_exists( 'cpbw_add_badge_oceanwp' ) ){

    function cpbw_add_badge_oceanwp() {
        global $product;
        ...
        // Apply badge
        $badge = cpbw_apply_product_badges( '', $product );
